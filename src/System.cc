...
System::System(std::vector<cv::Mat>& imgRGBOut, const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),mbActivateLocalizationMode(false),
        mbDeactivateLocalizationMode(false)
{
    // Output welcome message
    ...
    //Initialize the Tracking thread
    //(it will live in the main thread of execution, the one that called this constructor)
	MonoSlamPainter = new MonoPainter::PainterObject(imgRGBOut, mpMap, strSettingsFile);
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpKeyFrameDatabase, strSettingsFile, mSensor, MonoSlamPainter);
...
cv::Mat System::TrackMonoPainter(const cv::Mat &im, const cv::Mat &mask, const double &timestamp)
{
    if(mSensor!=MONOCULAR)
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular." << endl;
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonoPainter(im,mask,timestamp);
