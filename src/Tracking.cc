...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, MonoPainter::PainterObject* AnyPainter):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpViewer(NULL),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0), monopainter(AnyPainter)
{
    // Load camera parameters from settings file
...
cv::Mat Tracking::GrabImageMonoPainter(const cv::Mat &im, const cv::Mat &mask, const double &timestamp)
{
    mImGray = im;
    cv::Mat imMask = mask;
	...
	//Add the original rgb image to each frame
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
    {
        mCurrentFrame = Frame(mImGray,imMask,timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,im);		//the original RGB image is added to each frame's data
        monopainter->UpdatePainter(mCurrentFrame);
    }
    else
    {
        mCurrentFrame = Frame(mImGray,imMask,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,im);	//the original RGB image is added to each frame's data
		monopainter->UpdatePainter(mCurrentFrame);
    }	
    Track();
	//Here laid the main code for MonoPainter
	monopainter->MaskTheImage();
	if(!mCurrentFrame.mTcw.empty()){monopainter->ProjectMapPoints(mCurrentFrame, 0.5, 100);}
    return mCurrentFrame.mTcw.clone();

}

MonoPainter::PainterObject* Tracking::GetPainterObject()
{
	return monopainter;	
}
